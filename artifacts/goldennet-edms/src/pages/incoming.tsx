...
import { Search, FileText, Filter, Eye, Edit, Download, Inbox } from "lucide-react";
...
export default function Incoming() {
  ...
  const incomingDocs = documents?.filter(doc => doc.type === "وارد");
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
        ...
        <span>/</span>
        <span className="text-foreground font-semibold">المراسلات الواردة</span>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-1">المراسلات الواردة</h1>
            <p className="text-muted-foreground">استعرض جميع الكتب والوثائق الواردة للمؤسسة</p>
          </div>
        </div>
        <div className="bg-card border border-border px-4 py-2 rounded-xl font-bold text-lg shadow-sm">
          العدد الكلي: <span className="text-blue-500">{incomingDocs?.length || 0}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {isLoading ? (
          ...
          <div className="col-span-full py-24 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="text-xl font-bold mb-2">لا توجد مراسلات واردة حالياً</h3>
            <p className="text-muted-foreground text-sm max-w-xs">لم يتم العثور على أي مراسلات واردة. سيتم إظهارها هنا بعد ورودها.</p>
          </div>
        ) : (
          filteredDocs?.map((doc, idx) => (
            <motion.div
              ...
              key={doc.id}
              className="bg-card rounded-2xl p-6 border border-border shadow-sm hover:shadow-xl hover:border-blue-500/30 transition-all group flex flex-col h-full relative overflow-hidden"
            >
              ...
              <div className="mt-6 flex items-center justify-between gap-2 pt-2">
                 <Link href={`/document/${doc.id}`} className="flex-1">
                   <button className="w-full flex items-center justify-center gap-2 py-2 bg-blue-500/10 text-blue-600 hover:bg-blue-600 hover:text-white rounded-lg transition-colors font-semibold text-sm">
